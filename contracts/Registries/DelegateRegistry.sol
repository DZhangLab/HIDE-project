...
contract DelegateRegistry is Registry {
  ...
  /**
   * @dev Delete the entry of a delegate
   *
  ...
  function deleteDelegate(string memory _did) public returns (bool success) {
    ...
    for (i = 0; i != delegateToUsers[_did].length; ++i) {
      ...
      while (
        !StringUtils.equal(userToDelegates[delegateToUsers[_did][i]][j], _did)
      ) {
        ++j;
      }
  ...
  /** @dev Checks if a delegate can control a specific user
   *
   * Returns true if _delegateDid has _userDid in its user's mapping
   */
  function userIsDelegate(string memory _userDid, string memory _delegateId)
    public
    view
    returns (bool)
  {
    string[] memory users = lookupUsers(_delegateId);
    for (uint256 i = 0; i < users.length; i++) {
      if (
        keccak256(abi.encodePacked(users[i])) ==
        keccak256(abi.encodePacked(_userDid))
      ) {
        return true;
      }
    }
