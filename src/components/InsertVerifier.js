...
import VerifierRegistry from '../artifacts/contracts/VerifierRegistry.json';
...
const InsertVerifier = () => {
    ...
    async function insert() {
        ...
        if (!checkDid(did)) {
            setResult(`DID of length ${did.length} needs to be at least ${verificationNum}`);
            return;
        }

        // Ensures the contractKey meets the verification
        if (!checkContractKey(contractKey)) {
            setResult(`Contract key of length ${contractKey.length} needs to be at least ${verificationNum}`);
            return;
        }
        ...
        if (typeof window.ethereum !== "undefined") {
            ...
            const contract = new ethers.Contract(verifierRegistryAddress, VerifierRegistry.abi, signer);
